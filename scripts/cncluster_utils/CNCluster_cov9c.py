...
class CNClusterExact:
  ...
  def fit_one_window(self, varstart, varstop, varid, ncarriers):
    ...
    win=CNWindow_cov9b.CNWindow(self.combined_id, varid, varstart, varstop, cn1.cn,  self.nocl_max, ncarriers, self.verbose)
  ...
  def fit_mixture(self):
    ...
    for ii in range(self.clus_info.shape[0]):
      sys.stderr.write(str(self.clus_info.varstart.values[ii])+'\t'+str(self.clus_info.varstop.values[ii])+'\t'+str(self.clus_info.varid.values[ii])+'\t'+str(self.clus_info.info_ncarriers.values[ii]))
  ...
  def make_call_mixture(self, fits):
    ...
    if ncomp>1:
      ...
      ret=fits2.loc[fits2.nocl==1].sort_values(by=['dipp', 'info_ncarriers'], ascending=[True, False]).head(n=1)
      qual='PASS'
    else:
      ret=fits2.loc[fits2.nocl==1].sort_values(by=['info_ncarriers'], ascending=False).head(n=1)
      qual='LOWQUAL'
    ret['qual']=qual
    ...
    info_ncarriers=ret['info_ncarriers'].values[0]
    info=ret['info'].values[0]+';DIP_P='+str(dipp)+';INFO_NCARRIERS='+str(info_ncarriers)+';STARTS='+starts+';STOPS='+stops+';NVAR='+str(nvar)+';CIPOS='+ptspos+';CIEND='+ptsend
  ...
  def fit_generic(self, outf1):
    ...
    outstr=self.chrom+'\t'+str(fit['combined_id'].values[0])+'\t'+str(fit['varstart'].values[0])+'\t'+str(fit['varstop'].values[0])+'\t'+str(fit['nocl'].values[0])+'\t'+str(fit['qual'].values[0])+'\t'+str(fit['info'].values[0])+'\t'+cn1+'\n'
    outf1.write(outstr)
    return 


  def check_outliers_all(self):
      if self.carriers is not None and self.carriers.shape[0]>0:
