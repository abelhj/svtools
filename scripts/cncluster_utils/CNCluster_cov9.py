...
class CNClusterExact:
  ...
  def make_call_mixture(self, fits):
    ...
    if ncomp>1:
      ...
      qual='LOWQUAL'
    ret=ret[['combined_id', 'varid', 'varstart', 'varstop', 'region', 'nocl', 'bic', 'icl', 'sep', 'offset', 'info', 'cn_mad', 'cn_med', 'info_ncarriers']]
    ret['is_rare']=False
    ret['n_outliers']=0
    ...
    ret['info']=ret['info']+';STARTS='+starts+';STOPS='+stops+';NVAR='+str(nvar)+';CIPOS='+ptspos+';CIEND='+ptsend
    return [qual, ret]
  ...
  def fit_generic(self, outf1):
    ...
    if  qual=='LOWQUAL' and (self.ncarriers<0.0025*self.nsamp or self.ncarriers<10):
      ...
      if fit_rare is not None:
        fit=fit_rare
    fit.to_csv('fit2.csv')
    self.cndata.head().to_csv('head.csv')
    cn=self.cndata[['varid', 'id', 'cn']].copy().merge(fit, on='varid')
    if sum(cn.id!=self.subjlist)==0:
      cn1='\t'.join(cn['cn'].map(str))
    fit['info']=fit['info']+';SEP='+str(fit['sep'].values[0])+';OFFSET='+str(fit['offset'].values[0])+';CN_MED='+str(fit['cn_med'].values[0])+';CN_MED='+str(fit['cn_mad'].values[0])+';N_OUTLIERS='+str(fit['n_outliers'].values[0])
  ...
  def make_call_rare(self, fits):
    ...
    [ptspos, ptsend]=[str(minstart-varstart)+','+str(maxstart-varstart),
                      str(minstop-varstop)+','+str(maxstop-varstop)]
    fits['info']='STARTS='+starts+';STOPS='+stops+';NVAR='+str(nvar)+';CIPOS='+ptspos+';CIEND='+ptsend+';CN_CARRIER_MED='+str(fits['carrier_med'].values[0])
    fits=fits[['combined_id', 'varstart', 'varstop', 'region', 'varid', 'nocl', 'bic', 'icl', 'sep', 'offset', 'info', 'cn_mad', 'cn_med', 'n_outliers', 'qual']]
    return fits
